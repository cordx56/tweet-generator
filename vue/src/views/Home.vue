<template>
  <b-container class="home">
    ...
    <p>
      <strong>Detail/Bug report: <a href="https://github.com/cordx56/tweet-generator" target="_blank">cordx56/tweet-generator</a>.</strong>
    </p>
    ...
    <b-card v-if="sentence" header="生成結果" border-variant="info" header-bg-variant="info" header-text-variant="white" align="center">
      ...
      <b-card-text>
        <b-button variant="primary" size="sm" v-bind:href="tweetLink" target="_blank">
          <font-awesome-icon :icon="['fab', 'twitter']"></font-awesome-icon> ツイート
        </b-button>
      </b-card-text>
    </b-card>
    ...
    <b-card v-if="$route.query.successfully_generated" header="学習成功" border-variant="success" header-bg-variant="success" header-text-variant="white" align="center">
      <b-card-text>Successfully Generated!</b-card-text>
      <b-card-text>いっぱい遊んでいってね。楽しんでもらえたらうれしいな。ほめてもらえるともっとうれしいかも……</b-card-text>
    </b-card>
    <b-card v-if="$route.query.successfully_deleted" header="失敗" border-variant="success" header-bg-variant="success" header-text-variant="white" align="center">
      <b-card-text>正常に削除されました。</b-card-text>
    </b-card>
    <b-card v-if="$route.query.error_unknown" header="失敗" border-variant="danger" header-bg-variant="danger" header-text-variant="white" align="center">
      <b-card-text>不明なエラーです。しばらくしてからもう一度お試しください。</b-card-text>
    </b-card>
    <b-card v-if="$route.query.error_24hour_constraint" header="失敗" border-variant="danger" header-bg-variant="danger" header-text-variant="white" align="center">
      <b-card-text>モデルを生成できるのは24時間に1度のみです。</b-card-text>
    </b-card>
    <b-card v-if="$route.query.error_unregistered" header="失敗" border-variant="danger" header-bg-variant="danger" header-text-variant="white" align="center">
    ...
    <p>ログインしてツイートを学習させると、あなたや他の人もあなたのツイートを生成して遊べるようになります。</p>
    <p>
      <b-button variant="danger" size="sm" :href="deleteLink">
        <font-awesome-icon :icon="['fab', 'twitter']"></font-awesome-icon> 学習削除
      </b-button>
    </p>
...
export default {
  ...
  metaInfo() {
    return {
      meta: [
        { name: 'og:image', content: API_BASE_URL + '/v1/tweetgen/genImage/' + ((this.$route.params.screenName) ? this.$route.params.screenName : '') }
      ]
    }
  },
  methods: {
    onGenerate(evt) {
      ...
      this.$axios.get(API_BASE_URL + "/v1/tweetgen/genText/" + this.genForm.screenName, { params: { length: this.genForm.length, startWith: this.genForm.startWith } }).then((response) => {
        if (response.status) {
        ...
        if (error.response && error.response.data.message) this.errorMsg = error.response.data.message
